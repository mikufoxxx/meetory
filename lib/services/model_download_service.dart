...
class ModelDownloadService {
  ...
  static Future<bool> areAllModelsDownloaded() async {
    ...
    final assetsModelsDir =
        Directory(path.join(appDir.path, 'assets', 'models'));
  ...
  /// 下载单个模型
  static Future<bool> downloadModel(String modelKey,
      {String sourceType = 'mirror',
      String? customMirror,
      Function(double,
              {String? speed, String? downloadedSize, String? totalSize})?
          onProgress}) async {
    try {
      ...
      if (sourceType == 'direct') {
        ...
        final customUrl = directUrl.replaceFirst(
            'https://github.com', 'http://$customMirror/https://github.com');
      ...
      final appDir = await _getAppDirectory();
      final assetsModelsDir =
          Directory(path.join(appDir.path, 'assets', 'models', targetDir));
          ...
          if (response.statusCode == 200) {
            ...
            await for (final chunk in response.stream) {
              ...
              if (onProgress != null && contentLength > 0) {
                ...
                if (downloadedBytes > 1024 * 1024) {
                  ...
                  downloadedText =
                      '${(downloadedBytes / 1024).toStringAsFixed(1)} KB';
                } else {
            ...
            if (isArchive) {
              ...
              await _extractArchive(
                  Uint8List.fromList(bytes), assetsModelsDir.path, fileName);
            } else {
              // 直接保存文件
  ...
  static Future<bool> downloadAllModels(
    ...
    for (int i = 0; i < models.length; i++) {
      ...
      final success = await downloadModel(modelKey,
          sourceType: sourceType,
          customMirror: customMirror, onProgress: (progress,
              {String? speed, String? downloadedSize, String? totalSize}) {
        if (onProgress != null) {
          final overallProgress = (i + progress) / models.length;
          onProgress(modelKey, overallProgress,
              speed: speed,
  ...
  static Future<void> _extractArchive(
      Uint8List archiveBytes, String targetPath, String fileName) async {
    try {
      Archive archive;
  ...
  /// 从本地目录导入模型文件
  static Future<bool> importModelsFromDirectory(String sourceDirectory,
    ...
    try {
      ...
      for (int i = 0; i < models.length; i++) {
        ...
        await for (final entity in sourceModelDir.list(recursive: true)) {
          if (entity is File) {
            final relativePath =
                path.relative(entity.path, from: sourceModelDir.path);
            final targetFile =
                File(path.join(targetModelDir.path, relativePath));
  ...
  static Future<void> clearAllModels() async {
    try {
      ...
      if (await assetsModelsDir.exists()) {
        ...
        for (final targetDir in modelTargetDirs.values.toSet()) {
          final modelDir =
              Directory(path.join(assetsModelsDir.path, targetDir));
